...
% work sheet 1-A 345kV Vertical double circuit wo ground wires
% coordinates;
global iter iter2 T82 iter3
%% Step 1 - Setup tower coordinates and additional data
Vnom=345;%kV
...
hs=7;%m midspan
Dbundle=.467;%m distance in the bundle
Eo=1500;% kV/m 
W=iter2/100;% m Insulatorlength
span=335;%m 
top2crosam1=2.7;%m
top2crosam2=9.3;%m
top2crosam3=15.3;%m
...
I=30;%keraunic level tunderstorm days per year
%% Step 3 Define or determine Stroke incidence to earth
N=iter3/10;0.12*I; %flash density per km2 per year Eq. 12.4.1
...
NL=N*((X(2)-X(1))+4*hg^1.09)/10;%Line Flashes per 100km Eq. 12.4.6
%% Step 6 Look up Flashover Voltage od insulation strings at each phase
% (6mus) % Equations in Fig 12.6.3
t=6;%mus
K1=0.4*W;
K2=0.71*W;
Vs6=1000*(K1+K2/t^0.75)*Kt;%kV
...
%% Step 9 Compute single radius in all phases without Corona effect
for j=3:8
...
% Step 12 Compute minimum stroke current for shielding failure Eq. 12.5.6
for j=3:8
...
for j=3:8
Nsf(j)=(N/10)*(Xs(j))*(ProbMINsf(j)-ProbMAXsf(j))/2; %shielding failures/100km/yr
end
% Step 24 Compute total shielding failures 
NSft=sum(Nsf); %shielding failures/100km/yr %outages/100km.yr (shield failure)
% Step 25;
Nbf=NL-NSft; %Total flashes/100km/yr to be used in backflashover assessment

%% ----- Back-flashover calculations (EPRI 1982) Two Point Method
%% STEP 1: Insulator flasover voltage kV at 2museg
t=2;%museg
K1=0.4*W;
K2=0.71*W;
Vs2=1000*(K1+K2/t^0.75)*Kt;%KV
Vs2x=820*W*Kt; %Eq. 12.10.12
%% STEP 2: Insulator flasover voltage kV at 6museg
Vs6;
Vs6x=585*W*Kt;%Eq. 12.10.13
%% STEP 3: Multiply Vs2 by 1.8
Vsn2=1.8*Vs2;
%% STEP 4: COMPUTE SHIELD WIRE CORONA RADIUS
% Corona effect in shield wire
x = fsolve(@(x) x*log((2*Y(1))/x)-1.8*Vs2/Eo,[1],optimoptions('fsolve','Display','off'));
Rcor=x;%m Figure 12.5.3 Eo=1500kv/m
%% STEP 5 Compute self-surge and mutial impedance of each shield wire
Zcor(1,1)=60*sqrt(log(2*Y(1)/R(1))*log(2*Y(1)/Rcor));%with high corona Eq. 12.5.4
%Zcor(1,1)=Z(1,1); No corona
%% Step 6 Compute combined surge impedance (two shield wires)
b(1,2)=abs(X(1)-X(2));
a(1,2)=sqrt(b(1,2)^2+(2*Y(1))^2);
%Z(1,1)=60*log(2*Y(1)/R(1));% ohms
Z(1,2)=60*log(a(1,2)/b(1,2));% ohms
Zs=(Z(1,2)+Zcor(1,1))/2;% ohm Eq. 12.5.5A Eq. 12.5.6
...
Z(j,1)=60*log(a(j,1)/b(j,1));
Z(j,2)=60*log(a(j,2)/b(j,2));
k(j)=(Z(j,1)+Z(j,2))/(Z(1,2)+Zcor(1,1));% coupling factor 
end% Eq. 12.5.7
...
%% STEP 12 set impulse footing resistance
Rg=iter;%ohm impulse footing resistance Fig. 12.6.1
...
%% STEP 17 per unit tower top voltage at 2mus
...
Vr2=(alphaR*ZI/(1-psi))*(1-psi*taot/(1-psi));%    Eq. 12.10.1
...
%% Step 29 select the lowst currents (all at 2mus)
%% Step 30 select the corresponding Vs 
Vs2;
%% Step 31 plot IC fluctuation, percentage of time
n=360;
for theta=1:n
Icreal(3,theta)=Icn2(3)*(Vs2-Vo*sin((360/n)*pi*theta/180-0))/Vs2;%Phase A
Icreal(4,theta)=Icn2(4)*(Vs2-Vo*sin((360/n)*pi*theta/180+2*pi/3))/Vs2;%Phase B
Icreal(5,theta)=Icn2(5)*(Vs2-Vo*sin((360/n)*pi*theta/180-2*pi/3))/Vs2;%Phase C
Icreal(6,theta)=Icn2(6)*(Vs2-Vo*sin((360/n)*pi*theta/180-2*pi/3))/Vs2;%Phase C'
Icreal(7,theta)=Icn2(7)*(Vs2-Vo*sin((360/n)*pi*theta/180+2*pi/3))/Vs2;%Phase B'
Icreal(8,theta)=Icn2(8)*(Vs2-Vo*sin((360/n)*pi*theta/180+0))/Vs2;%Phase A'
th(theta)=theta;
end
for theta=1:n
...
%Step 37
Np.*ProbBF;%outages/100km.yr (only backflshover, per phase)
%Step 38 
Tbf=sum(Np.*ProbBF);%outages/100km.yr (only backflshover);
%step 39
Tbfplussh=Tbf+NSft;%outages/100km.yr (backflshover and shield failure)
T82(iter,iter2,iter3)=Tbf;
% figure
% plot(th,Icreal(3,:),'r-',th,Icreal(4,:),'b-',th,Icreal(5,:),'g-',th,Icreal(6,:),'r--',th,Icreal(7,:)...
%     ,'b--',th,Icreal(8,:),'g--');
% figure
% plot(th,A,'r-')
% figure
