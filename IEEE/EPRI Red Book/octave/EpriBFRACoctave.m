function [Tbf] = EpriBFRACoctave(Rg,W,Ng)
...
Vo = sqrt(2)*Vnom/sqrt(3);  %kV
    ...
    for j=3:8
        x = fsolve(@(x) x*log((2*hf(j))/x)-Vs6/Eo,[1]);
    ...
    x = fsolve(@(x) x*log((2*Y(1))/x)-Vsn2/Eo,[1]);
    ...
    %Zt1 = 30*log(2*((2.7)^2+(11/2)^2)/(11/2)^2)
    %Zt2 = 30*log(2*((6.6)^2+(2.7/2)^2)/(2.7/2)^2)
    %Zt3 = 30*log(2*((6.0)^2+(2.7/2)^2)/(2.7/2)^2)
    %Zt4 = 30*log(2*((24)^2+(10/2)^2)/(10/2)^2)        
    ...
    % Step 38 Total backflashover per 100 km per year 
     ...
     %disp('RED Book')
    Tbf = sum(Tbfp)/(0.6*NL); %outages/100km.yr (only backflshover)
