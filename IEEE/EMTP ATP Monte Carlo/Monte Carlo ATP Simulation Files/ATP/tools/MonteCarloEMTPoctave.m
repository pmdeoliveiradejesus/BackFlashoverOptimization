%% ------------------------------
%Calculo de TAsas de Salida Forzada por Flameo Inverso (Montecarlo=
% Universidad de los Andes
% Paulo M. De Oliveira, pm. deolive
%%
clc
clear all
pkg load statistics
    ...
    delete('bfr01.mat');
%% Datos de diseño
Rg=35;%Resistencia de Impulso de Puesta a Tierra, ohms 
...
D = 10000; %Cantidad de descargas aleatorias;
%%% Otros datos
...
for i = 1:D
    ...
    angB=(ang-120);
    angC=(ang-240);
    I =  (lognrnd(log(33300),0.605)); %Corriente aleatoria de la descarga, kA       
    tf = (lognrnd(log(2),0.494))*10^-6; %Tiempo de frente de onda de la descarga, microsegundos
  ...
  fclose(fid); 
    ...
    system('pl42mat.vbs'); %Convierte los resultados de ATP de PL4 a .mat a través de un .vbs y GTPPL
    load('bfr01.mat');  
...
% Resultados
disp('   Longitud(m) Ensayos     #Freq.  Probabilidad Tasa');
[   w(1)/100   kk-1 nfo(kk-1,1) P(kk-1,1) T(kk-1,1);
     w(2)/100   kk-1 nfo(kk-1,2) P(kk-1,2) T(kk-1,2);
     w(3)/100   kk-1 nfo(kk-1,3) P(kk-1,3) T(kk-1,3);
     w(4)/100   kk-1 nfo(kk-1,4) P(kk-1,4) T(kk-1,4);
     w(5)/100   kk-1 nfo(kk-1,5) P(kk-1,5) T(kk-1,5);
]
